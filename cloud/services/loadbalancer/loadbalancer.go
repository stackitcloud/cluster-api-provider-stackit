...
func TargetForMachine(machineName string, addresses []cloud.Address) (cloud.LoadBalancerTargetInput, error) {
...
func EnsureForMachine(
	...
	loadBalancerID, err := ResolveID(ctx, cloudClient, stackitCluster)
	if err != nil {
		return "", err
	}
	if loadBalancerID != "" {
	...
	target, err := TargetForMachine(machineName, addresses)
	if err != nil {
		return "", err
	}

	loadBalancer, err := cloudClient.EnsureAPIServerLoadBalancer(ctx, APIServerInput(stackitCluster, []cloud.LoadBalancerTargetInput{target}))
	if err != nil {
		return "", err
	}
